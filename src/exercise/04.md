...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=setmaxo%40gmail.com).

## What I've learned

# Components

- A component is a function that returns something renderable. Custom component
  Rule, they have to be capitalized because of the Babel compilation to
  `React.createElement('message', null, null)`. If not, the browser won't
  recognize the message HTML tag

- Remember that I can't interpolate statements, only expresions (check previous
  excercise).

- Using the React Dev Tools, in the Component tab, if I check the difference by
  the First Extra Credit, I will find that the DOM goes as follows if I include
  the `helloElement` created using `React.createElement` and when just executing
  the `message` function that just renders the `<div>`
...
Clearly React manages both situations differently. If I create `byeElement`
using `React.createElement` it will end up like this
...
So, I'm using `React.createElement` to render a tag with type `message`, which
is a function that returns a renderable result. That sounds pretty much like a
custom Component.

- As Babel is a tool that runs between the code I deploy and the browser, I can
  check at any time at the Elements tab of the React Dev Tools and see the
  compiled code.

- This PropType thing is a built-in feature of React. I think it could work if
  you are doing something quick and small but for bigger apps better using
  TypeScript. Besides, this could be quite expensive in terms of performance and
  that's why it's only available in development. If I change the name of the
  react package imported using unpkg from development to production, Babel will
  automatically remove all references to PropTypes.

  - `<></> = <Fragment></Fragment>` I need to include these in case I want to
    return something more complex than a single tag. The React.Fragment
    component allow me to do it, as `<View />` does in React Native.

  - The main motivation for the Fragment or View is to be able to render more
    than one component side by side. If I just try to render this:
  ...
  it will be an error message on sight. That JSX will be compiled by Babel and
  try to run something like
  `React.createElement(Message, {greeting: "Hello", subject: "World"})...` and
  then I realized I need to put the second Message component, but where? I need
  to be able to declare both Messages as a list of children of a root component
  and that's why we use `Fragment`
