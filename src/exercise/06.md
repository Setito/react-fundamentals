...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=setmaxo%40gmail.com).

## My Notes

# Forms

- `onSubmit` - `submitEvent` - `submitEvent.target` = `ref to <form> DOM` -
  `ref to form elements`

- `event.preventDefault()` is used to prevent the default behavior of form
  submit events which refreshes the page. This default behavior is doing a GET
  HTTP request to the current URL containing as URL parameters the
  (`name=value`) of the inputs int the form submited. Preventing default, will
  do instead a POST request to the URL (have to be a better way here)

- run `console.dir(event.target)` to see the HTML as a collection of properties

# useRef

- This hook returns an object that can store any value in its current property
  and can be updated at any time, remaining consistent between re-renders. If we
  assign this `userNameRef` reference to a DOM element such as an input in a
  form, we can access its current value at any time making sure the value we are
  accessing will be consistent even though between renders. We can learn more
  about this hook in the future, for this case I don't think is the best
  approach. We should be maintaining the current state of the input into a state
  variable. Even better, have an `<Input />` component.

# useState

- In React setState, is recomended to use the value and update the value using
  the `set` function. This is because we should use the function provided, need
  to investigate why this is like this.
