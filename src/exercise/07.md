...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=setmaxo%40gmail.com).

# My Notes

## Array Methods

- Write more about those methods

## Key Prop

- React is not able to determine what happend with the elements of an array
  between re-renders if a `key` prop wasn't provided for each one of the array
  elements. That is, we can remove, add or change the order and React won't be
  able to determine what you did since the input is not associated with any of
  the array elements. If we have a `key` prop assigned, then when we re-render a
  component because this one has been edited, included or removed, React have a
  way to unmount the element that matches the value into `key` and mounting it
  again (running any clean up functions required in the proccess).

- To demostrate this issue and its solution, I liked the example of shufling the
  array elements and re-rendering everything every 1 or 2 secs (extra credit 1)

- Using the index of the array to identify each element is not a good approach
  because the index associated to the item can easily change when added a new
  item to the array, delete items in between or shufling the array. Better using
  something that identifies the item no matter how the user plays with it.

- In summary, the `key` prop works as a way that React has to keep track of
  elements between re-renders and make sure any change in the state of those
  elements is correctly re-render every time. Also notice that this is only an
  issue when the elements maintain a state.

- Remember how this list will be compiled by Babel:
...
Only `null` values are assigned to the key, which is fine in order to render the
list on screen but not enough in case subsecuent updates are comming and React
can't track it down as expected.

## Investigate more about optimize react re renders

## Array elements maintaining state
